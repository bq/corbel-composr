...
describe.only('Engine', function() {
  ...
  describe('Services checking', function() {
    ...
    it.skip('resolves before timeout', function(done) {
    ...
    it.skip('rejects one endpoint and it should retry upto ' + retries + ' times and finally login', function(done) {
      ...
      engine.init({})
        ...
        .then(function() {
          engine.abort();
          setTimeout(function() {
            console.log(spyOnComposrInit.callCount); 
    ...
    it.skip('rejects two endpoints and it should retry upto ' + retries + ' times and finally login', function(done) {
      ...
      engine.init({})
        ...
        .then(function() {
          setTimeout(function() {
            ...
            console.log(retries); 
            console.dir(spyOnComposrInit.args[0]); 
            console.dir(spyOnComposrInit.callCount); 
            console.dir(spyOnTemporalRetry.callCount); 
            expect(spyOnServicesUp.callCount > retries-1).to.be.true;
            expect(typeof spyOnComposrInit.args[0][0] === 'object').to.be.true;
            expect(spyOnComposrInit.args[0][1] === false).to.be.true;
            expect(spyOnComposrInit.calledOnce).to.be.true;
