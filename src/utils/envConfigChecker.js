...
try {
  //fs.accessSync('../config/' + environmentFileConfig);
  var envConfigFile = require('../config/' + environmentFileConfig);
  configPrepared = _.defaults(envConfigFile, config);
} catch (e) {
  configPrepared = config;
  logger.warn('File src/config/' + environmentFileConfig + ' is not present, default config will be used');
}
...
function checkIfConfigurationValuesAreValuesByDefault(){
  ...
  if(defaultValueCheckerBool){
    ...
    logger.warn(defaultValueCheckerLog);
  }
}
